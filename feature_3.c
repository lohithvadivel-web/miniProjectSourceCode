...
int  cmpByBalance(const void *a, const void *b);
...
// ─── Sort: comparator — balance descending ────────────────────────────────────
int cmpByBalance(const void *a, const void *b)
{
...
// ─── Sort and display (menu option 10) ───────────────────────────────────────
void sortAndDisplay(FILE *fPtr)
{
    struct clientData all[100];
    struct clientData client = {0};
    int count = 0;
    int result;
    int i;

    // Load all active records into the array
    rewind(fPtr);
    while (!feof(fPtr))
    {
        result = fread(&client, sizeof(struct clientData), 1, fPtr);
    ...
    if (count == 0) { puts("No active accounts to display."); return; }

    unsigned int sortChoice;
    printf("\nSort by:\n"
           "  1 - Balance (highest to lowest)\n"
           "  2 - Last name (A to Z)\n"
           "? ");
    scanf("%u", &sortChoice);
    ...
    if (sortChoice == 1)
    {
        qsort(all, count, sizeof(struct clientData), cmpByBalance);
        printf("\n--- Accounts sorted by Balance (descending) ---\n");
    }
    else if (sortChoice == 2)
    {
        qsort(all, count, sizeof(struct clientData), cmpByLastName);
        printf("\n--- Accounts sorted by Last Name (A-Z) ---\n");
    }
    else
    {
        puts("Invalid sort choice.");
        return;
    }

    printf("\n%-4s  %-6s%-16s%-11s%10s  %s\n",
           "Rank", "Acct", "Last Name", "First Name", "Balance", "Status");
    printf("%-4s  %-6s%-16s%-11s%10s  %s\n",
           "----", "----", "---------", "----------", "-------", "------");

    for (i = 0; i < count; i++)
    {
        printf("%-4d  %-6d%-16s%-11s%10.2f  %s\n",
               i + 1,
               all[i].acctNum,
               all[i].lastName,
               all[i].firstName,
               all[i].balance,
               all[i].locked ? "LOCKED" : "Active");
    }
    printf("\nTotal accounts: %d\n", count);
}
